        ...
        if (articleContract && events) {
            (async () => {
                try {
                    ...
                    const final = articles.map((article, index) => { return { ...article, id : index+1, cover: uris[index].image, article: `${uris[index].attributes[5].value}/${uris[index].attributes[3].value}` } })
                    setArticles(final)
                }catch(err){
                    console.log(err)
                }
